...
module.exports = {
	...
	run: async (client, message, args) => {
        ...
        //console.log(rmod)
        if (!args[2]) return message.channel.send('Something Is Missing, format is w!lbassistadd @[Member] [Assists] [Player Username]')
        const igncheck = maca.playerStatus(args[2])

        if (igncheck == undefined) return message.channel.send('Something Is Missing, format is w!lbassistadd @[Member] [Assists] [Player Username]')
        ...
        let prof = await profileModel.findOneAndUpdate({ userID: membero.id }, { assists: args[1],  ign: usernamed, uuid: uuid })
        message.channel.send('Player ' + usernamed + ' has been added to the leaderboards!')
        const objectify = client.commands.get('albupdate')
