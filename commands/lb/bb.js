...
module.exports = {
	...
	run: async (client, message, args) => {
        ...
        console.log('Tested')
        //console.log(rmod)
        if (!args[2]) return message.channel.send('Something Is Missing, format is w!lbbbadd @[Member] [Blocks Broken] [Player Username]')
        const igncheck = maca.playerStatus(args[2])

        if (igncheck == undefined) return message.channel.send('Something Is Missing, format is w!lbbbadd @[Member] [Blocks Broken] [Player Username]')
        ...
        let prof = await profileModel.findOneAndUpdate({ userID: membero.id }, { bb: args[1],  ign: usernamed, uuid: uuid })
        message.channel.send('Player ' + usernamed + ' has been added to the leaderboards!')
        const objectify = client.commands.get('bblbupdate')
